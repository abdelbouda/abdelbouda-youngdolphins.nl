...
export default function Hero({ settings }: HeroProps) {
  ...
  // Bubbles only on desktop/tablet, with scale transform to avoid layout thrashing
  const showBubbles = !isMobile;
  const bubbleCount = isTablet ? 2 : 4;

  return (
    <section id="hero" className="relative pt-32 pb-2 lg:pt-40 lg:pb-4 overflow-hidden scroll-mt-24 bg-aquatic">
      {/* Animated background bubbles – only on larger screens */}
      {showBubbles && [...Array(bubbleCount)].map((_, i) => (
        <motion.div
          key={i}
          className="absolute rounded-full bg-secondary/10 w-32 h-32 md:w-40 md:h-40"
          initial={{
            x: Math.random() * 100 + '%',
            y: '110%',
            scale: 0.3,
      ...
      {/* Wave animation – only on desktop (>=1024px) */}
      {!isMobile && !isTablet && (
        <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] pointer-events-none">
          <svg
            className="relative block w-[200%] h-24 text-white fill-current opacity-50"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 max-w-3xl mx-auto">
            {pricingCards.map((card, i) => (
              <motion.a
                ...
                className={`${card.color} ${card.text} p-4 rounded-xl shadow-premium border border-slate-100 transition-all flex flex-col items-center group relative cursor-pointer overflow-hidden min-h-[120px] will-change-transform`}
              ...
              >
                ...
                <div
                  className={`flex flex-col items-center transition-all duration-300 ${
                    activeCard === i ? 'opacity-0 -translate-y-4' : 'opacity-100 translate-y-0'
                  }`}
                >
                  ...
                  <div className="flex items-baseline gap-0.5">
                    ...
                    <span className="text-[10px] font-bold text-primary/80">
                      {card.popular ? t('hero_per_month') : t('hero_per_lesson')}
                    </span>
                  </div>
                  <div
                    className={`mt-2 px-3 py-1.5 rounded-lg font-black text-[10px] uppercase tracking-wider transition-all ${
                      card.popular ? 'bg-white text-primary' : 'bg-primary text-white'
                    }`}
                  >
                    {t('hero_register_btn')}
                  </div>
                ...
                <div
                  className={`absolute inset-0 p-4 flex flex-col items-center justify-center text-center transition-all duration-300 ${
                    activeCard === i ? 'opacity-100 translate-y-0 z-20' : 'opacity-0 translate-y-4 pointer-events-none'
                  }`}
                >
